//! **cbmman** — interactive TUI manager for
//! [codebase-memory-mcp](https://github.com/DeusData/codebase-memory-mcp).
//!
//! A terminal UI for indexing and querying *codebase knowledge graphs* without
//! hand-writing MCP JSON tool calls. Two implementations ship together:
//! `cbmman.sh` (bash + [tuish](https://github.com/alganet/tuish)) and the Rust
...
//! # What it does
//!
//! * **Projects** — list indexed projects (branch, nodes, edges, size, root
//!   path), then run per-project actions: index status, architecture overview,
//!   index coverage, git-diff impact (`detect_changes`), re-scan, delete.
//! * **Scan / Index** — add a repo (`moderate`/`full`/`fast`/`cross-repo`),
//!   optional artifact persistence (`.codebase-memory/graph.db.zst`).
//! * **Graph server (UI)** — start/stop the 3D UI, set port, open in browser.
//! * **Processes & resources** — live `codebase-memory-mcp` process + artifact
//!   monitor (auto-refresh).
//! * **Configuration** — edit persisted settings (`auto_index`, `ui_port`, …).
//! * **Graph queries** — search graph/code, get snippet, trace paths, Cypher,
//!   architecture, schema, ingest traces.
//! * **ADR** — manage architecture decision records; **Maintenance** — version,
//!   help, update, cache diagnostics.
//!
//! # Crate API
//!
//! The binary is the primary interface (`cargo run`). The library surface is
//! intentionally small — see [`App`], [`Project`], and the `run_*` helpers.
//!
//! # Env
//!
//! * `CBM_BIN` — path to the `codebase-memory-mcp` binary (default: PATH).
//! * `CBM_CACHE_DIR` — cache dir for the process monitor.
//!
//! # Website
//!
//! <https://handyutils.github.io/cbmman/>
//!
//! # Upstream
//!
...
pub fn run_tool_raw(tool: &str, args: &str) -> JobResult {
    let bin = cbm_bin();
...
pub fn run_tool(tool: &str, args: &str) -> JobResult {
...
pub fn format_mcp(stdout: &str) -> Vec<String> {
...
pub fn parse_projects(stdout: &str) -> Vec<Project> {
    let mut out = Vec::new();
    let i = stdout.find('{').unwrap_or(0);
...
pub fn parse_config(stdout: &str) -> Vec<(String, String)> {
    let mut out = Vec::new();
...
pub fn run_app(terminal: &mut Terminal<ratatui::backend::CrosstermBackend<std::io::Stdout>>, app: &mut App) -> io::Result<()> {
    while app.running {
